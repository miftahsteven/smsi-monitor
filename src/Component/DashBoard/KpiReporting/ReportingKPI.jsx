import React, { Fragment, useEffect, useState } from 'react';
import axios from 'axios';
import { Card, CardBody, CardHeader, Col, Table } from 'reactstrap';
import { Link } from 'react-router-dom';
import { ContactNumber, Country, Date, Name, Status } from '../../../Constant';
import { H5, P, Image } from '../../../AbstractElements';

const ReportingKPI = () => {
  const [generalData, setGeneralData] = useState([]);
  const { DataASN } = generalData;
  useEffect(() => {
    axios.get(`${process.env.PUBLIC_URL}/api/general.json`).then((res) => setGeneralData(res.data.result));
  }, []);
  var images = require.context('../../../assets/images', true);
  const dynamicImage = (image) => {
    return images(`./${image}`);
  };
  return (
    <Fragment>
      {DataASN &&
        <Col xl="12" className="manage-invoice-sec box-col-12">
          <Card>
            <CardHeader className="pb-0">
              <H5>Daftar ASN Kemenag</H5>
            </CardHeader>
            <CardBody>
              <div className="manage-invoice-table">
                <div className="item">
                  <div className="table-responsive manage-invoice">
                    <Table className="table-bordernone">
                      <thead>
                        <tr>
                          <th>{Name} </th>
                          <th>Terakhir Posting</th>
                          <th>Akun</th>
                          <th>Media Sosial</th>
                          <th>Asal Lembaga</th>
                          <th>Jumlah Share/Bulan</th>
                          {/* <th>{Status}</th> */}
                        </tr>
                      </thead>
                      <tbody>
                        {
                          DataASN.map((data, i) => {
                            return (
                              <tr key={i}>
                                <td>
                                  <span className='p-b-50'>{data.srNo}</span>&nbsp;&nbsp;
                                  <div className="t-title">
                                    <div className="d-inline-block align-middle">
                                      <Link>
                                        <span className="f-w-500 font-secondary">{data.name}</span>
                                        {/* berikan konten dibagian bawah nama, konten dari data.content */}
                                        <p className="mb-0">{data.content}</p>
                                      </Link>
                                    </div>
                                  </div>
                                </td>
                                <td>
                                  <P>{data.date}</P>
                                </td>
                                <td>
                                  <span>{data.project}</span>
                                </td>
                                <td>
                                  <P>{data.status}</P>
                                </td>
                                {/* <td>
                                  <P>{data.contactNumber}</P>
                                </td> */}
                                <td>
                                  <P>{data.country}</P>
                                </td>
                                <td>
                                  <P>{data.amount}</P>
                                </td>
                                {/* <td>
                                  <div className="progress-showcase">
                                    <div className="progress sm-progress-bar">
                                      <div className={data.progressBar} role="progressbar" style={{ width: data.styleWidth }} aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                                    </div>
                                  </div>
                                </td> */}
                              </tr>
                            );
                          })
                        }
                      </tbody>
                    </Table>
                  </div>
                </div>
              </div>
            </CardBody>
          </Card>
        </Col>}
    </Fragment>
  );
};
export default ReportingKPI;