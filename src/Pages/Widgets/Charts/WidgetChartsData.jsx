...
const primary = localStorage.getItem("default_color") || ConfigDB.primary_color;
    ...
    chart: {
      ...
      events: {
        animationEnd: function (chartCtx) {
          const newData = chartCtx.w.config.series[0]?.data ? [...chartCtx.w.config.series[0].data] : [];
          ...
          window.setTimeout(() => {
           try {
            chartCtx.updateOptions({
                ...
                xaxis: {
                min: newData[0]?.x || undefined,
                max: newData[newData.length - 1]?.x || undefined,
                },
                subtitle: {
                text: `${parseInt(getRangeRandom({ min: 1, max: 20 }))}%`,
                },
              },
              false,
              false
...
export const ApexLineChartOption = {
  options: {
    chart: {
      ...
      events: {
        animationEnd: function (chartCtx) {
          const newData1 = chartCtx.w.config.series[0]?.data ? [...chartCtx.w.config.series[0].data] : [];
          const newData2 = chartCtx.w.config.series[1]?.data ? [...chartCtx.w.config.series[1].data] : [];
          newData1.shift();
          newData2.shift();
          window.setTimeout(() => {
        try {
